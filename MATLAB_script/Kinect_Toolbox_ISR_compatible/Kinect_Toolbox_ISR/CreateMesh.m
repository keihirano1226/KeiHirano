...
clear, clc, close all
N=60;
imD = sprintf('data/%04d-d.pgm', N);
imR = sprintf('data/%04d-c1.jpg', N);
imd = read_disparity(imD);
imR=imread(imR);

calibFile=load('MyTBCalibRes_6.mat');
distFile = load('MyTBCalibDistEst6');
calib.dc_beta=distFile.meanMat;
    calib.dc_alpha=[0 distFile.a1];
dK =[calibFile.INd.fc(1) 0 calibFile.INd.cc(1)
...
disp_k=undistort_disparity(u(:)'-1,v(:)'-1, imd(:)',calib);
...
% alpha=0;
xout=ProjectPoints(P3D,calibFile.Tdep2camout,calibFile.INc.fc,calibFile.INc.cc,calibFile.INc.kc,calibFile.INc.alpha);
