...
int main()
{
  ...
  /*
  std::ifstream stream("/home/kei/document/experiments/2019.06.21/data4/2DGround.csv");
  std::string line;
  ...
  //int j = 0;
  FILE *fp;
  fp=fopen("/home/kei/document/experiments/2019.06.21/data4/3DGround.csv","w");
  cv::Mat depthtest ;
  cv::Mat depthMat ;
  //depth画像を読み込む
  depthtest =  cv::imread( "/home/kei/document/experiments/2019.06.21/data4/depth_mirror/0000001135.png",2);
  ...
  */
  ...
  for ( row1 = 0; row1 < row ; ++row1  ) {
    ...
    for ( col = 0; col < 50; col = col + 2 ) {
      cv::Mat depthtest ;
      cv::Mat depthMat ;
      depthtest =  cv::imread( "/home/kei/document/experiments/2019.02.27.test/Exdepth/" + oss.str() + ".png",2);
      depthtest.convertTo(depthMat, CV_32FC1);
