...
csvfile2 = sys.argv[1] + "verticalPara2.csv"
...
point = Ground[100:101].values
#Direction1 = body[["MidHipX","MidHipY","MidHipZ"]]
#point = Direction1[0:1].values
...
original1 = dfall[["MidHipX","MidHipY","MidHipZ"]]
original = original1[0:1].values
print(original)
original[0,2] = 0
print(original)
originalFrame = np.zeros((len(dfall),76))
j = 0
for j in range(len(dfall)):
    for i in range(25):
        originalFrame[j,3*i+1] = original[0,0]
        originalFrame[j,3*i+2] = original[0,1]
        originalFrame[j,3*i+3] = original[0,2]

dfall = dfall - originalFrame
