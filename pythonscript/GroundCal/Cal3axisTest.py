...
"""
G0 = np.array([9.98*10**(-3),10.48*10**(-3),36.42*10**(-3)])
G1 = np.array([10.23*10**(-3),410.04*10**(-3),35.86*10**(-3)])
G2 = np.array([210.01*10**(-3),9.78*10**(-3),35.94*10**(-3)])
origin = np.array([-0.9,0,0])
vecg0 = G0 - origin
vecg1 = G1 - origin
vecg2 = G2 - origin
...
vecg0_rotated = np.dot(R_inv, vecg0.T)
vecg1_rotated = np.dot(R_inv, vecg1.T)
vecg2_rotated = np.dot(R_inv, vecg2.T)
Kinectg0 = vecg0_rotated + original
Kinectg1 = vecg1_rotated + original
Kinectg2 = vecg2_rotated + original
print(Kinectg0)
print(Kinectg1)
print(Kinectg2)
pointvector = np.r_[Kinectg0,Kinectg1,Kinectg2]
point = np.reshape(pointvector,(3, 3))
dfKinect = pd.DataFrame(point)
dfKinect.columns = ["x","y","z"]
dfKinect.to_csv(sys.argv[1] + "KinectGround3point.csv",index = 0)
"""
#体節位置座標データの読み込み
